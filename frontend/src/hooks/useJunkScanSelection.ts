...
import { GetJunkSelectionSummary } from '../../wailsjs/go/main/App';
import type { ScanItem } from '../types';
import { selectedIdsFromItems } from '../utils/scanItems';

const LARGE_SCAN = 5000;

export function useJunkScanSelection(items: ScanItem[], pageActive: boolean) {
  ...
  // Rebuild local selection when scan results are replaced.
  useEffect(() => {
    if (items.length === 0) {
      scanKeyRef.current = '';
    ...
    // Do not bump selectionRev here — it used to change every virtual row key and
    // corrupt row positions after cleanup refresh.
  }, [items]);
  ...
  useEffect(() => {
    ...
    if (large) {
      void GetJunkSelectionSummary().then((s) => {
    ...
    summaryCache.current = next;
    setSummary(next);
  }, [pageActive, large, selectionRev, selectedIds]);
